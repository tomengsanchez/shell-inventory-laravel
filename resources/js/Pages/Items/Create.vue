    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ form_type === 'edit' ? 'Edit Item' : 'Add Item' }}
            </h2>
        </template>
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        <form @submit.prevent="addItem" class="mt-6 space-y-6">
                            ...
                            <div v-if="form_type === 'edit'">
                                ...
                                <InputLabel for="item_type_id" value="Item Types" />
                                <select id="item_type_id" v-model="form.item_type_id" class="mt-1 block w-full">
                                    <option v-for="item in data" :value="item.item_type_id" :key="item.id">
                                        {{ item.item_type_id }}
                                    </option>
                                </select>
                                <InputError class="mt-2" :message="form.errors.item_type_id" />
                    ...
                    <div class="item-info">
                            <h2>Item Information</h2>
                            <p><strong>Item Id:</strong> {{  item_info.item_id }}</p>
                            <p><strong>Item Name:</strong> {{ item_info.item_name }}</p>
                            <p><strong>Item Type ID:</strong> {{ item_info.item_type_id }}</p>
                        </div>
                </div>
            </div>
        </div>
